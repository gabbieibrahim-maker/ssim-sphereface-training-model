...
# when using pdb, use c to exit and n to move to next line (does not run until you are on the next line)
...
class PatchPairDataset(Dataset):
    ...
    def findSSIM(artifactArray, gtArray): # this is ranging between 0 and 1
        # print(f"the maximum pixel value for the gt array is {gtArray.max()}')
        dataRange = PatchPairDataset.findDataRange(gtArray) # ssim requires data range because 
        # it uses the maximum possible pixel value to be used for constrast and luminance comparison 

        ssimCalc =  structural_similarity(artifactArray, gtArray, data_range = dataRange)
        validSSIM = True
        ...
        return ssimCalc, validSSIM

    @staticmethod

    def findDataRange(gtArray):
        return gtArray.max() - gtArray.min()

    def __getitem__(self, idx):

        s = self.samples[idx]
        # s is 
        # {'condition_path': '/vast/tibrahim/jil202/data/train/knee/pd_cor_1.5T/1001353_ANISO_read3.5/patch_0000_0001_0001.nii.gz',
        #  'target_path': '/vast/tibrahim/jil202/data/train/knee/pd_cor_1.5T/md1001353/patch_0000_0001_0001.nii.gz', 
        # 'prompt': 'Denoised and biascorrected coronal 1.5T knee Proton Density MRI of resolution 0.44 x 0.44 x 3 mm.', 
        # 'task': 'artifact2denoised',
        #  'anatomy': 'knee',
        #  'artifact': 'aniso'}
        
        condition = _load_patch(s["condition_path"], self.patch_shape) # shape is [1, 96, 96, 16]
        target = _load_patch(s["target_path"], self.patch_shape) # shape is [1, 96, 96, 16]

        if self.augment is not None:
            condition = self.augment(condition) # shape is [1, 96, 96, 16]
        
        # import pdb
        # pdb.set_trace()
        conditionLayer = condition[:, :, :, 0] # shape is now [1, 96, 96]
        targetLayer = target[:, :, :, 0]

        conditionSqueeze = conditionLayer.squeeze() # shape is now [96, 96]
        targetSqueeze = targetLayer.squeeze() 
        # current type here is torch.float32

        conditionNumpy = conditionSqueeze.numpy()
        targetNumpy = targetSqueeze.numpy()

        ssimCalc, validSSIM = PatchPairDataset.findSSIM(conditionNumpy, targetNumpy)
        ...
        # cannot just return None for ssimCalc, as python DataLoader function will crash if None type
        return condition, target, s["prompt"], target_type, s["anatomy"], s["artifact"], ssimCalc, validSSIM
...
def getloader_3d_patches(
    ...
    return train_loader, val_loader #getdataset_3d_patches gets us our train set and val set (training and testing data)
...
if __name__ == "__main__":
    ...
    for condition, target, prompts, target_types, *_ in train_loader: # return here
        print(
